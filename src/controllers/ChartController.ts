...
export const getChartDataController = async (req: Request, res: Response): Promise<any> => {
    try {
        ...
        const excelData: IexcelData[] = xlsx.utils.sheet_to_json(workbook.Sheets[sheetName])
        ...
        return res.status(500).json({
            error: "Internal server error in getChartData", e
        })
    }
}


export const getFilteredDataController = async (req: Request, res: Response): Promise<any> => {
    try {
        ...
        if (from && !age  && !gender  && !to ) {
            ...
            filteredData = excelData.filter(data => data.Day >= from && data.Day <= to);
        }
        else if (from && age && !to  && !gender ) {
            filteredData = excelData.filter(data => data.Age == age && data.Day === from);

        }
        else if (from && gender && !to  && !age ) {
            filteredData = excelData.filter(data => data.Day == from && data.Gender === gender);
        }
        else if (to && age && !gender && !from) {
            filteredData = excelData.filter(data => data.Age == age && data.Day === to);
        }
        else if (to && gender && !from  && !age ) {
            filteredData = excelData.filter(data => data.Gender == gender && data.Day === to);
        }
        else if (age && gender && !to  && ! from ) {
            filteredData = excelData.filter(data => data.Age == age && data.Gender === gender);
        }

        else if (to && from && age && !gender ) {
            filteredData = excelData.filter(data => data.Day <= to && data.Day >= from && age === data.Age);
        }
        else if (gender && to && from && !age ) {
            filteredData = excelData.filter(data => (from <= data?.Day) && (to >= data?.Day) && (gender === data.Gender));
        }
        else if (gender && age && from && !to ) {
            filteredData = excelData.filter(data => (from === data?.Day) && (gender >= data?.Gender) && (age === data.Age));
        }

        else if (age && to && gender && ! from ) {
            filteredData = excelData.filter(data => (to >= data?.Day) && (age === data.Age) && gender === data.Gender);
        }

        else if (from  && to && age && gender) {
            filteredData = excelData.filter(data => (from <= data?.Day) && (to >= data?.Day) && (age === data.Age) && gender === data.Gender);
        }
        if (selectedBar) {
        ...
        console.log(e);
        return res.status(500).json({
            error: "Internal server error in getFilteredData", e
        })
    }
}